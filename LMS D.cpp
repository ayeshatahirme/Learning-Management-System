...
void EditCourse(string codeList[], string nameList[], int crtHrsList[], int semList[], int h)
{
	...
	while (t == false)
	{
		...
		if (a == true && x == true && y == true && z == true)
		{
			...
			semList[t] = semester;
			cout << "\n Course has been edited successfully." << endl
				 << endl;
			t = true;
		}
		else
...
void DeleteCourse(string codeList[], string nameList[], int crtHrsList[], int semList[], int h)
{
	...
	cout << " Your course " << courseCode << " has been deleted." << endl
		 << endl;
}
		...
		if (a == true && b == true && c == true)
		{
			for (int t = 0; t <= l; t++)
			{
				if (year[l] == regNo[t] && dept[l] == stdNamesList[t] && roll_no[l] == stdRegNoList[t])
				{
					...
					cout << " Student of registration number " << year[l] << "-" << dept[l] << "-" << roll_no[l] << " has been deleted." << endl
						 << endl;
					l++;
				}
				else
			...
			if (isValidCourseCode(courseCode) == true)
			{
				...
				for (int t = 0; t < l; t++)
				{
					if (year[l] == regNo[t] && dept[l] == stdNamesList[t] && roll_no[l] == stdRegNoList[t])
					{
						...
						{
							coursecode[t] = '\0';
							cout << " Course code of registration number " << year[l] << "-" << dept[l] << "-" << roll_no[l] << " is unregistered." << endl
								 << endl;
							l++;
						}
						else
...
int main()
{
	...
	if (file && file1)
	{
		...
		cout << ""
			 << "	  **"
			 << "Welcome to university of Engineering and Technolohgy"
			 << ""
			 << "**" << endl;
